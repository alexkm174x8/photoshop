...
extern void inv_img_grey_horizontal(const char* mask, const char* path) {
    ...
    int padding = (4 - (ancho * 3) % 4) % 4;
    unsigned char* pixels = malloc(ancho * alto);

    for(int i=0; i<alto; i++){
        for(int j=0; j<ancho; j++){
            unsigned char b = fgetc(image), g = fgetc(image), r = fgetc(image);
            pixels[i * ancho + j] = 0.21*r + 0.72*g + 0.07*b;
        }
    ...
    for(int i=0; i<alto; i++){
        for(int j=ancho-1; j>=0; j--){
            ...
            fputc(pxl, outputImage); fputc(pxl, outputImage); fputc(pxl, outputImage);
        }
        for(int p=0; p<padding; p++) fputc(0, outputImage);
    }
    free(pixels); fclose(image); fclose(outputImage);
}
...
extern void inv_img_color_horizontal(const char* mask, const char* path) {
    ...
    int padding = (4 - (ancho * 3) % 4) % 4;
    unsigned char *b = malloc(ancho*alto), *g = malloc(ancho*alto), *r = malloc(ancho*alto);
    ...
    for(int i=0; i<alto; i++){
        for(int j=ancho-1; j>=0; j--){
            int idx = i * ancho + j;
            fputc(b[idx], outputImage); fputc(g[idx], outputImage); fputc(r[idx], outputImage);
        }
        for(int p=0; p<padding; p++) fputc(0, outputImage);
    }
    free(b); free(g); free(r); fclose(image); fclose(outputImage);
}
...
extern void desenfoque(const char* input_path, const char* name_output, int kernel_size) {
    ...
    for (int i = 0; i < alto; i++) {
        ...
        for (int x = 0; x < ancho; x++) {
            unsigned char b = in_rows[i][x*3], g = in_rows[i][x*3+1], r = in_rows[i][x*3+2];
            unsigned char gray = 0.21*r + 0.72*g + 0.07*b;
            in_rows[i][x*3] = in_rows[i][x*3+1] = in_rows[i][x*3+2] = gray;
        }
    }
